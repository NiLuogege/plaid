...
class DribbbleModule(private val activity: ShotActivity, private val shotId: Long) {
    ...
    @Provides
    @Singleton
    fun provideContext(): ShotActivity = activity
    ...
    @Provides
    @Singleton
    fun chromeFader(activity: ShotActivity): ElasticDragDismissFrameLayout.SystemChromeFader {
    ...
    @Provides
    @Singleton
    fun shotViewModelFactory(coroutinesContextProvider: CoroutinesContextProvider) =
        ShotViewModelFactory(
            shotId,
            provideShotsRepository(),
            provideGetShareShotInfoUseCase(activity),
            coroutinesContextProvider
        )

    private fun provideGetShareShotInfoUseCase(context: Context) =
        GetShareShotInfoUseCase(provideImageUriProvider(context))

    private fun provideImageUriProvider(context: Context) = ImageUriProvider(context)
}
